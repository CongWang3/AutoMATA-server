...
<script type="text/javascript">
    ...
    var download_example = function() {
        ...
        const fileUrl = "./example/useModel_testing_Example.txt"; // 文件的实际路径
        const fileName = "useModel_testing_Example.txt"; // 下载时保存的文件名
    ...
    var download_example_2 = function(){
        ...
        // window.open("./example/train_example.zip")
        const fileUrl = "./example/useModel_cnn_Example.pth"; // 文件的实际路径
        const fileName = "useModel_cnn_Example.pth"; // 下载时保存的文件名  cnn模型
    ...
    var change = function(){ // 选择som时，增加一个上传pkl文件的div
        var obj = document.getElementById("model_type");
        // var sele = obj.options;
        var index = obj.selectedIndex;
        if (index == 4){  // autoencoder
            document.getElementById("upload_model").style.display="none";
            document.getElementById("upload_winmap").style.display="none";
            document.getElementById("upload_som").style.display="none";
            document.getElementById("upload_encoder").style.display="";
            document.getElementById("upload_classifier").style.display="";
        }else if (index == 6){  // som
            document.getElementById("upload_model").style.display="none";
            document.getElementById("upload_som").style.display="";
            document.getElementById("upload_winmap").style.display="";
            document.getElementById("upload_encoder").style.display="none";
            document.getElementById("upload_classifier").style.display="none";
        }else{  // other
            document.getElementById("upload_som").style.display="none";
            document.getElementById("upload_model").style.display="";
            document.getElementById("upload_winmap").style.display="none";
            document.getElementById("upload_encoder").style.display="none";
            document.getElementById("upload_classifier").style.display="none";
        }
    ...
    var change = function(){ // 选择som时，增加一个上传pkl文件的div
        ...
        if (index == 4){  // autoencoder
            ...
            document.getElementById("upload_encoder").style.display="";
            document.getElementById("upload_classifier").style.display="";
            document.getElementById("upload_un_semi_model").style.display="none";
            document.getElementById("upload_un_semi_scaler").style.display="none";
            ...
            document.getElementById("upload_un_semi_model").style.display="none";
            document.getElementById("upload_un_semi_scaler").style.display="none";
        }else if (index == 10 || index == 8 || index == 9 || index == 11){  
            // un_semi
            document.getElementById("upload_model").style.display="none";
            document.getElementById("upload_winmap").style.display="none";
            document.getElementById("upload_encoder").style.display="none";
            document.getElementById("upload_classifier").style.display="none";
            document.getElementById("upload_un_semi_model").style.display="";
            document.getElementById("upload_un_semi_scaler").style.display="";
...
<section id="banner">
    <div class="container padding-medium-2">
        <div class="hero-content">
            ...
            <h2 class="display-2 fw-semibold">Use your own model</span></h2>
            <nav class="breadcrumb">
                ...
                <span class="breadcrumb-item active" aria-current="page">Use</span>
            </nav>
        </div>
    </div>
</section>
...
<section id="service" class="padding-medium">
    ...
    <div class="container">
        ...
        <form id="form" class="train_form" name="train_form" method="post" action="modelUse.php" onsubmit="JavaScript: return checkInput()" enctype="multipart/form-data"> <!-- onsubmit="javascript:return checkInput()"-->
            

            <div class="pb-3" style="margin-bottom: 22px;">
                <label>1. Choose model</label> <br>
                <select class="param_select" id="model_type" name="model_type" onChange="change()"> <!-- style="font-size:17px; font-style:italic; -->
                    <option selected="selected" value="0">CNN</option>
                    <option value="1">LSTM</option>
                    <option value="2">RNN</option>
                    <option value="3">MLP</option>
                    <option value="4">AutoEncoder</option>
                    <option value="5">Transformer</option>
                    <option value="6">SOM</option>
                    <option value="7">RBFNN</option>
                    <!-- <option value="8">GAN encoder</option> -->
                     <!-- 一审 半监督和监督的测试数据集需要label和样本名，无监督的测试数据集不需要label和样本名 -->
                    <option value="8">VAE</option>
                    <option value="9">DeepCluster</option>
                    <option value="10">LadderNetwork</option>
                    <option value="11">Pseudo-Labeling</option>
                </select>
            </div>

            <div class="pb-3">
                <!-- 一审 -->
                <label>2. Upload testing set</label> <label id="note" style="display:none;">(Note: Do not contain sample labels or names)</label>
            ...
            <div class="pb-3"  id="upload_model">  <!--style="display: ;"-->
                <label>3. Upload model file</label>
            ...
            <div class="pb-3" style="display: none;margin-bottom: 22px;" id="upload_som">
                <label>3. Upload model file</label>
                <input type="file" id="upload_file_6" name="upload_file_6" class="form-control" data-text="Choose your file">
                <!-- <button type="button" class="btn btn-primary example" onclick="download_example_2()">Example</button> -->
            </div>

            <div class="pb-3" style="display: none;margin-bottom: 22px;" id="upload_winmap">
                <label>4. Upload winmap file</label>
                <input type="file" id="upload_file_3" name="upload_file_3" class="form-control" data-text="Choose your file">
                <!-- <button type="button" class="btn btn-primary example" onclick="download_example_2()">Example</button> -->
            </div>

            <!-- autoencoder -->
            <div class="pb-3" style="display: none;margin-bottom: 22px;" id="upload_encoder">
                <label>3. Upload encoder model file</label>
                <input type="file" id="upload_file_4" name="upload_file_4" class="form-control" data-text="Choose your file">
                <!-- <button type="button" class="btn btn-primary example" onclick="download_example_3()">Example</button> -->
            </div>

            <div class="pb-3" style="display: none;margin-bottom: 22px;" id="upload_classifier">
                <label>4. Upload classifier model file</label>
                <input type="file" id="upload_file_5" name="upload_file_5" class="form-control" data-text="Choose your file">
                <!-- <button type="button" class="btn btn-primary example" onclick="download_example_2()">Example</button> -->
            </div>

            <!-- 一审 半监督和无监督模型 -->
            <div class="pb-3" style="display: none; margin-bottom: 10px;" id="upload_un_semi_scaler">
                <label>3. Upload scaler file</label>
                <input type="file" id="upload_file_6" name="upload_file_6" class="form-control" data-text="Choose your file">
                <!-- <button type="button" class="btn btn-primary example" onclick="download_example_3()">Example</button> -->
            </div>

            <div class="pb-3" style="display: none;margin-bottom: 22px;" id="upload_un_semi_model">
                <label>4. Upload model file</label>
                <input type="file" id="upload_file_7" name="upload_file_7" class="form-control" data-text="Choose your file">
                <!-- <button type="button" class="btn btn-primary example" onclick="download_example_2()">Example</button> -->
            </div>
