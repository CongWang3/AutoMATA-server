...
- **首装不依赖镜像仓库出网**：在介质完整、Docker 已就绪的前提下，适合 **内网、涉密、禁外网** 终端。
- **版本冻结清晰**：`docker save` 与安装包版本绑定，可附 **digest/镜像 ID 清单**，便于验收与审计。
- **不抢占用户日常带宽**：大镜像已在介质中，安装过程主要受磁盘与 `docker load` 速度影响，而非在线拉取抖动。
- **甲方可控分发**：软件走 **光盘/U 盘/内网文件服务器** 即可，无需为用户开放公网 registry 账号。

### 本方案（完全离线 tar）缺点

- **介质体积大**：MySQL/Redis/胖后端/前端等叠加后，**数 GB 级** 常见；分卷、压缩与下载托管成本高。
- **升级路径重**：用户通常需取得 **新离线包** 并重新 `load`（若不做复杂的「增量离线补丁」）。
- **工程与发布链路更长**：需维护 **save 脚本、`load` 顺序、安装器**（Win/Linux 可能不同）；若用户机 **未预装 Docker**，还需在契约中明确 **是否随包提供 Docker 安装介质**（再等体积与许可问题）。
- **平台仍为 Linux 容器**：tar 为 **`linux/amd64` 镜像**；与联网方案一致，**不**等同「为 Windows 单独构建一套 Windows 容器镜像」。

### 与「联网 pull」方案对比

| 维度 | 完全离线 tar（本文档） | 联网 pull（见另一 spec） |
|------|------------------------|--------------------------|
| **首装是否必须连公网/镜像库** | **否**（依赖介质内 tar） | **是**（或连内网私服执行 pull） |
| **安装包/介质体积** | **大** | **小** |
| **版本升级** | 通常整包或新 tar | **`pull` + 重启栈**，常更省增量流量 |
| **交付与审计** | 离线清单 + 签收场景天然匹配 | 依赖 registry 管理与网络侧日志 |
| **适用场景** | 涉密、无外网、介质拷贝 | 有网环境、可接受首次拉取 |
| **实现与运维侧复杂度** | **中高**（打包、安装器、合规材料） | **相对低**（构建 push + 文档） |
