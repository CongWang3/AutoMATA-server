...
- **分发轻**：桌面安装包主要含壳程序与 compose/脚本；镜像由用户本机在有网时拉取，不必把数 GB 打进一个安装包。
- **更新友好**：版本发布以 **registry tag/digest** 为准，用户 **`compose pull` 可利用层增量**，通常比反复下发整块离线 tar 更省流量。
- **发布链路简单**：CI 构建并 **`docker push`** 后即可被客户端拉取，与现有 **GHCR** 类流程一致。
- **镜像元数据标准**：`linux/amd64` 单平台或日后扩平台，均由仓库 manifest 表达，客户端无需区分「安装包里是第几卷 tar」。

### 本方案（联网 pull）缺点

- **首装与更新强依赖网络**：必须能访问镜像仓库（或你们提供的**内网私服**）；纯断网机无法完成首次装全栈（除非事先在另一台机 pull 后再自行 export，那已接近离线交付形态）。
- **首次体验受带宽与仓库可用性影响**：胖后端镜像较大时，首次等待明显；需在产品文案与健康检查里管理预期。
- **供应链暴露于网络**：需考虑仓库鉴权、被墙镜像源、企业内部代理等（可通过镜像站或内网 registry 缓解）。

### 与「完全离线 tar」方案对比

| 维度 | 联网 pull（本文档） | 完全离线 tar（见另一 spec） |
|------|---------------------|-----------------------------|
| **首装是否必须连公网/镜像库** | 是（除非改为私网 registry，仍属「拉取」模型） | 否（介质内已含 tar，且 Docker 本身已具备时） |
| **安装包/介质体积** | 小 | 大（含多个 `docker save` 产物） |
| **版本升级** | **`pull` + 再起容器**，常享层增量 | 通常 **新离线包或新 tar 整包** |
| **交付与审计** | 以 registry tag/digest 为唯一来源 | 可在包内附带 **digest 清单**，离线签收清晰 |
| **适用场景** | 一般个人/实验室有网环境、可搭私服的企业 | 涉密、禁外网、仅 U 盘/内网介质拷贝 |
| **实现与运维侧复杂度** | **低**（维护 push 与文档） | **中高**（打包、分卷、`load` 顺序、安装器、可能捆绑 Docker 安装包） |

**共性**：二者在 **compose 拓扑、胖后端、同源 nginx、数据卷、桌面壳职责** 上应对齐；差异集中在 **镜像如何进入用户 Docker**。
