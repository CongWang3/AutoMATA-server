...
目标：在同一 FastAPI 服务内，避免长时间同步子进程阻塞事件循环；同时将“所有任务（训练+数据处理+分析）”的全局并发上限限制为 4，并保持“排队仍显示 Submitted”。
...
- **G2**：限制所有任务的**全局并发上限**（训练+数据处理+分析合计）≤ 4。
...
- 在真正启动外部脚本前，使用一个全局 `asyncio.Semaphore(4)` 进行并发限制。
...
- 由于本设计对所有任务做全局限流为 4，理论上同一进程内“等待子进程结束”的最大并发为 \(4\)。
- 为避免默认线程池过小导致 `to_thread` 排队，应采用 **专用 ThreadPoolExecutor**（或至少保证 executor 的 `max_workers >= 6`）来承载外部命令等待：
  - 推荐实现：应用启动时创建 `subprocess_executor = ThreadPoolExecutor(max_workers=4)`（可配置），并用 `loop.run_in_executor(subprocess_executor, subprocess.run, ...)` 执行。
...
引入一个全局并发信号量（建议放在服务层模块的单例区域或专门的 `api/services/concurrency.py`）：
...
本设计在**单进程** FastAPI/uvicorn 实例内提供严格限流（全局 ≤ 4）。
...
- 并发提交 6 个任务（可混合训练/处理/分析）：应有最多 4 个进入 Processing，其余保持 Submitted + `current_step=排队中...`
...
- **并发上限可观测（全局）**：使用可控外部命令或 mock runner，并发提交 6 个混合任务，断言同一时刻全局 Processing 数量 ≤ 4。
