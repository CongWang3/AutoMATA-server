...
class OntologyDataProcessor {
    ...
    private function parseFile() {
        ...
        // echo "$data";  // 待删
        $this->data = $data;
        // throw new Exception("{$this->data}");  // 待删
    }
    ...
    public function getFilteredData($page, $filters = []) {
        ...
        // $numericFields = ['Prediction Probability'];
            ...
            if ($min !== null || $max !== null) {
                $filteredData = array_filter($filteredData, function($row) use ($field, $min, $max) {
                    ...
                    if (!isset($row["{$field}"]) || $row["{$field}"] === '') {
                        // throw new Exception("跳过空值或未定义字段");  // 待删
    ...
    try {
        ...
        if (isset($_GET['export'])) {
            ...
            $filters = [
                ...
                'min_probability' => $_GET['min_probability'] ?? '',
                'max_probability' => $_GET['max_probability'] ?? ''
            ];
