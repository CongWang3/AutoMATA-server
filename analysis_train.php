...
<script type="text/javascript">
    ...
    var checkInput = function() {  // 检查后缀名 txt csv excel
        ...
        // if (document.getElementById("upload_val").style.display == ""){
        //     var files = ["upload_file_1", "upload_file_2", "upload_file_3"];
        // }else{
        //     var files = ["upload_file_1", "upload_file_3"];
        // }
        ...
        var lr = document.getElementById("seed").value;
        if (lr == ""){
        ...
        var lb = document.getElementById("labels").value;
        if (lb == ""){
            window.alert("Please input Label Number value");
            var train_form = document.getElementsByClassName("train_form")[0];
            train_form.focus();
            return false;
        }
    ...
    var download_example = function() {
        ...
        const fileUrl = "./example/analysis_train_example.zip"; // 文件的实际路径
        const fileName = "analysis_train_example.zip"; // 下载时保存的文件名
    ...
    var download_example_2 = function() {
        const fileUrl = "./example/analysis_train_example/group_info.txt"; // 文件的实际路径
        const fileName = "group_info.txt"; // 下载时保存的文件名
    ...
    // function  uploadVal(){
    //     document.getElementById("upload_val").style.display="";
    //     document.getElementById("kfold_val").style.display="none";
        
    // }
    // function  useKfold(){
    //     document.getElementById("upload_val").style.display="none";
    //     document.getElementById("kfold_val").style.display="";
...
<section id="banner">
    <div class="container padding-medium-2">
        <div class="hero-content">
            ...
            <h2 class="display-2 fw-semibold">Analyze and Train</span></h2>
            <nav class="breadcrumb">
                <a class="breadcrumb-item text-muted nav-link" href="#">Home</a>
                <!-- <a class="breadcrumb-item text-muted nav-link" href="index.php">Analyze and Train</a> -->
                <span class="breadcrumb-item active" aria-current="page">Analyze and Train</span>
            </nav>
        </div>
    </div>
</section>
...
<section id="service" class="padding-medium">
    ...
    <div class="container">
        ...
        <form id="form" class="train_form" name="train_form" method="post" action="analysis_train_2.php" onsubmit="JavaScript: return checkInput()" enctype="multipart/form-data"> <!-- onsubmit="javascript:return checkInput()"-->
            
            <div class="pb-3" style="margin-bottom: 22px;">
                <label>1. Select data type</label>
                <select class="param_select" id="data_type" name="data_type" onChange="change()"> <!-- style="font-size:17px; font-style:italic; -->
                    <option selected="selected" value="0">Read Counts</option>  <!--调用DESeq2-->
                    <option value="1">FPKM</option>  <!--调用limma-->
                </select>
            </div>

            <div class="pb-3" style="margin-bottom: 22px;">  <!--转为Gene Symbol-->
                <label>2. The gene nomenclature used in the input expression data</label> <br>
                <select class="param_select" id="first" name="geneMomen" onChange="change()"> <!-- style="font-size:17px; font-style:italic; -->
                    <option selected="selected" value="0">Gene Symbol</option>
                    <option value="1">Ensembl ID</option>
                    <option value="2">Gene ID</option>
                </select>

            </div>
            
            <!-- 分析数据时做转置并删除最后一行对数据文件，分析训练集 -->
             <div class="pb-3" style="margin-bottom: 10px; ">
                <label>3. Select strategy and upload files</label> <br/> <br/>
            ...
            <div class="pb-3 upload_dataset" style="margin-bottom: -18px;">
                <label>Upload training set (Only datasets labeled with categories 0/1/2 to 6 can be uploaded)</label>  <!-- 一审-->
            ...
            <div class="pb-3 split_dataset" style="margin-bottom: 22px; display: none;" >
                ...
                <input type="text" name="ratio" id="ratio" value="8:1:1" placeholder="8:1:1" class="form-control">
            </div>


            <!--KFold-->
            <div class="pb-3 kfold_dataset" style="margin-bottom: -18px; display: none;">
                <label>Upload training set</label>
            ...
            <div class="pb-3 kfold_dataset" id="kfold_val" style="display: none;" >
                ...
                <input type="text" name="kfold" id="kfold" value="3" placeholder="3" class="form-control">
            </div>
            ...
            <div class="pb-3" style="margin-bottom: 22px; margin-top: -22px;">
                <label>Upload sample information (Note: The sample information file should have the same sample order as the expression/training data file)</label>
                <input type="file" id="upload_file_info" name="upload_file_info" class="form-control example" data-text="Choose your file">
                <button type="button" class="btn btn-primary example" onclick="download_example_2()">Example</button>
            </div>

            <!-- <div class="pb-3" style="margin-bottom: 10px;">
                <label>3. Upload a training/expression set (i.e. analysis data)</label>
                <input type="file" id="upload_file_1" name="upload_file_1" class="form-control example" data-text="Choose your file">
                <button type="button" class="btn btn-primary example" onclick="download_example()">Example</button>
            </div>

            <div class="pb-3"  style="margin-bottom: 10px;">
                <label>4. Upload sample information (Note: The sample information file should have the same sample order as the expression/training data file)</label>
                <input type="file" id="upload_file_info" name="upload_file_info" class="form-control example" data-text="Choose your file">
                <button type="button" class="btn btn-primary example" onclick="download_example_2()">Example</button>
            </div>

            <div class="pb-3">
                <label>5. Validation set</label>  &nbsp;&nbsp;&nbsp;
            ...
            <div class="pb-3" style="margin-bottom: 22px;">
                <label>6. Upload testing set</label>
                <input type="file" id="upload_file_3" name="upload_file_3" class="form-control" data-text="Choose your file">
            </div> -->
             ...
             <div class="pb-3" style="margin-bottom: 22px;">
                <label>4. Choose model</label> <br>
                <select class="param_select" id="first" name="model_type" onChange="change()"> <!-- style="font-size:17px; font-style:italic; -->
                    <option selected="selected" value="0">CNN > CNN Excels at extracting discriminative features from 1D signals with local patterns (e.g., mass spectrometry)</option>
                    <option value="1">LSTM > Designed for modeling data with long-term dependencies </option>
                    <option value="2">RNN > Used for processing simple sequences with short-term dependencies</option>
                    <option value="3">MLP > A general-purpose benchmark model for non-linear classification of feature vectors without explicit structure</option>
                    <option value="4">AutoEncoder > Generates better feature representations for classification tasks through dimensionality reduction and denoising</option>
                    <option value="5">Transformer > Models the global context via the self-attention mechanism</option>
                    <option value="6">SOM > Suitable for discovering inherent cluster structures within data</option>
                    <option value="7">RBFNN > Suitable for fast learning and local approximation of small-scale tabular data</option>
                    <option value="8">All > All the above models are trained in parallel using the same set of data</option>
                </select>
            </div>

            <div class="pb-3">
                <label>5. Upload training hyperparameters</label> <br>
            </div>
            <div class="pb-3">
                <label>(1) Epoch</label>
                <input type="text" name="epoch" id="epoch" value="1" class="form-control">
            </div>
            <div class="pb-3">
            ...
            <div class="pb-3">
                ...
                <input type="text" name="es" id="es" value="1" class="form-control">
            </div>
            <div class="pb-3">
            ...
            <div class="pb-3">
                <label>(5) Label Number (Note: If you want to use multi-label classification, then the number of labels is greater than 2. Upper limit of 7.)</label>
                <input type="text" name="labels" id="labels" value="2" class="form-control">
            </div>
            <div class="pb-3">
                <label>(6) Loss Function</label>
                <select class="param_select" id="second" name="loss_type" onChange="change()"> <!-- style="font-size:17px; font-style:italic; -->
                    <option selected="selected" value="0">CrossEntropyLoss</option>
                    <option value="1">FocalLoss</option>
                    <option value="2">NLLLoss</option>
                </select>
            </div>
            <div class="pb-3">
                <label>(7) Optimizer</label>
                <select class="param_select" id="third" name="opz_type" onChange="change()"> <!-- style="font-size:17px; font-style:italic; -->
            ...
            <!-- 一审 -->
             <div class="pb-3" style="margin-bottom: 22px;">
                <label>(8) Random Seed</label>
                <input type="text" name="seed" id="seed" value="42" class="form-control">
            </div>


            <div class="pb-3" style="margin-bottom: 22px;">
                <label>6. The organism of the input data</label> <br>
                <select class="param_select" id="organism" name="organism" onChange="change()"> <!-- style="font-size:17px; font-style:italic; -->
            ...
            <div class="pb-3">
                <label>7. Input threshold for screening differential genes</label>  &nbsp;&nbsp;&nbsp; <br/>
            </div>
            <div class="pb-3">
                <label>(1) log2(FC) threshold</label>  &nbsp;&nbsp;&nbsp;
                <input  type="text" name="fc" id="fc" value="1" class="form-control">
            </div>
            <div class="pb-3" style="margin-bottom: 20px;">
                <label>(2) padj threshold</label>  &nbsp;&nbsp;&nbsp;
                <input type="text" name="padj" id="padj" value="0.05" class="form-control">
            </div> 
            
            <!-- 一审 新增-->
            <div class="pb-3" style="margin-bottom: 22px;">
                <label>8. The hypothesis correction method</label> <br>
                <select class="param_select" id="correction" name="correction" onChange="change()"> <!-- style="font-size:17px; font-style:italic; -->
