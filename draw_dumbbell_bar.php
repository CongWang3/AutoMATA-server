...
<script type="text/javascript">
    ...
    var download_example1 = function() {
        const fileUrl = "./example/draw_example/dumbbell_example.txt"; // 文件的实际路径
        const fileName = "dumbell_example.txt"; // 下载时保存的文件名
    ...
    var download_example2 = function() {
        const fileUrl = "./example/draw_example/dumbbell_barplot_example.txt"; // 文件的实际路径
        const fileName = "dumbbell_barplot_example.txt"; // 下载时保存的文件名
...
<section id="banner">
    <div class="container padding-medium-2">
        <div class="hero-content">
            ...
            <h2 class="display-2 fw-semibold">Dumbbell with Bar Plot</span></h2>
            <nav class="breadcrumb">
                ...
                <span class="breadcrumb-item active" aria-current="page">Dumbbell with Bar Plot</span>
            </nav>
        </div>
    </div>
</section>
...
<section id="service" class="padding-medium">
    <div class="container">
        <form id="form" class="draw_form" name="draw_form" method="post" action="draw_dumbbell_bar2.php" onsubmit="JavaScript: return checkInput()" enctype="multipart/form-data"> <!-- onsubmit="javascript:return checkInput()"-->
            
            <div class="pb-3">
                <label>1. Upload dumbbell data set</label>
                <input type="file" id="upload_file_1" name="upload_file_1" class="form-control example" data-text="Choose your file">
                <button type="button" class="btn btn-primary example" onclick="download_example1()">Example</button>
            </div>
            <div class="pb-3">
                <label>2. Upload bar data set (NOTE: Please make sure that the first clumn of the bar data set is the same as the first column of the dumbbell data set)</label>
                <input type="file" id="upload_file_2" name="upload_file_2" class="form-control example" data-text="Choose your file">
                <button type="button" class="btn btn-primary example" onclick="download_example2()">Example</button>
            </div>
            <div class="pb-3">
                <label>3. Input the label for the x-axis</label>
                <input type="text" name="x_label" id="x_label" placeholder="e.g. Number of Transitions" class="form-control">
            </div>
            <div class="pb-3" style="margin-bottom: 20px; margin-top: 20px;">
                <label>4. Input terms that you want to emphasize (NOTE: Please separate them by English comma, and entered terms should be included in the uploaded data set) </label>  &nbsp;&nbsp;&nbsp;
                <input type="text" name="mark_fams" id="mark_fams" placeholder="e.g. Notothenioid,Sebastidae,Liparidae,Zoarcidae,Pleuronectidae" class="form-control">
            </div>
            ...
            <div class="pb-3" style="text-align: center; margin-top: 30px;">
                ...
                <img src="images/dumbbell_bar_example.png" alt="img" style="margin: 0 auto; width: 800px; height: 1200px;">
            </div>
        </form>
