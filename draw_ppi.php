...
<script type="text/javascript">
    ...
    var checkInput = function() {  // 检查后缀名 txt csv excel
        ...
        var confidence = document.getElementById("show").value;
        if (confidence == ""){
            window.alert("Please input the number of nodes!");
            var draw_form = document.getElementsByClassName("draw_form")[0];
            draw_form.focus();
            return false;
        }

        var confidence = document.getElementById("thres").value;
        if (confidence == ""){
            window.alert("Please input threshold!");
            var draw_form = document.getElementsByClassName("draw_form")[0];
            draw_form.focus();
            return false;
        }
    ...
    var download_example = function() {
        const fileUrl = "./example/draw_example/ppi_example.txt"; // 文件的实际路径
        const fileName = "ppi_example.txt"; // 下载时保存的文件名
...
<section id="banner">
    <div class="container padding-medium-2">
        <div class="hero-content">
            ...
            <h2 class="display-2 fw-semibold">Protein-Protein Interaction Analysis</span></h2>
            <nav class="breadcrumb">
                ...
                <span class="breadcrumb-item active" aria-current="page">Protein-Protein Interaction Analysis</span>
            </nav>
        </div>
    </div>
</section>
...
<section id="service" class="padding-medium">
    <div class="container">
        <form id="form" class="draw_form" name="draw_form" method="post" action="draw_ppi2.php" onsubmit="JavaScript: return checkInput()" enctype="multipart/form-data"> <!-- onsubmit="javascript:return checkInput()"-->
            ...
            <div class="pb-3" style="margin-bottom: 22px;">
                <label>2. The organism of the input data</label> <br>
                <select class="param_select" id="organism" name="organism" onChange="change()"> <!-- style="font-size:17px; font-style:italic; -->
                    <option selected="selected" value="0">Mus musculus</option>
                    <option value="1">Bos taurus</option>
                    <option value="2">Homo sapiens</option>
                    <option value="3">Drosophila melanogaster</option>
                </select>
            </div>

            <div class="pb-3" style="margin-bottom: 22px;">
                <label>3. The nomenclature used in the data set</label> <br>
                <select class="param_select" id="nomenclature" name="nomenclature" onChange="change()"> <!-- style="font-size:17px; font-style:italic; -->" name="geneMomen" onChange="change()"> <!-- style="font-size:17px; font-style:italic; -->
                    <option selected="selected" value="0">Gene Symbol</option>
                    <option value="1">Ensembl ID</option>
                    <option value="2">ENTREZID</option>
                </select>

            </div>

            <div class="pb-3" style="margin-bottom: 20px;">
                <label style="margin-bottom: 16px;">4. Input filtered threshold of score for PPI analysis</label>  &nbsp;&nbsp;&nbsp;
                <!-- <label>pvalue (NOTE: The threshold range is [0, 1]) </label>  &nbsp;&nbsp;&nbsp; -->
                <input  type="text" name="thres" id="thres" value="400" class="form-control">
                <!-- <label>qvalue (NOTE: The threshold range is [0, 1]) </label>  &nbsp;&nbsp;&nbsp;<input type="text" name="qvalue" id="qvalue" value="1" class="form-control"> -->
                
            </div>

            <div class="pb-3" id="show_div" style="margin-bottom: 20px;" >
                <label>5. Only gene names with more than this number of nodes are shown in the figure</label>  &nbsp;&nbsp;&nbsp;
                <input type="text" name="show" id="show" value="5" class="form-control">
            </div>

            <div class="pb-3" style="margin-bottom: 20px;">
                <label>6. Plot type</label>  &nbsp;&nbsp;&nbsp;
                <input type="radio" name="type" value="linear" checked >Linear &nbsp;
                <input type="radio" name="type" value="kk"> KK &nbsp;
                <input type="radio" name="type" value="stress" > Stress &nbsp; 
            ...
            <div class="pb-3" style="text-align: center; margin-top: 30px;">
                ...
                <img src="images/ppi_example.png" alt="img" style="margin: 0 auto; width: 1100px; height: 350px;">
            </div>
        </form>
